...
3) **Configure and run test** (host-driven via SSH; set `AE_STATE_DB=/home/ae/state/controller.db` for CLI lookups)
   - Controller:
     ...
     AE_AGENT_API_PORT=9110 AE_AGENT_API_TOKEN=ci-token \
     AE_NETWORK_NAME=ae-net AE_DOCKER_NETWORK=ae-net \
     python3 -m ae.controller --loop --specs specs/ --metrics-port 9108 > /var/log/ae-controller.log 2>&1 &
     ```
   - Workers:
     ```
     AE_CONTROLLER_URL=http://192.168.152.10:9110 \
     AE_AGENT_TOKEN=ci-token \
     AE_NODE_ID=worker1 \
     AE_NODE_ADVERTISE_IP=192.168.152.11 \
     AE_AGENT_ENDPOINT=http://192.168.152.11:9109 \
     AE_AGENT_HEARTBEAT_SECONDS=10 \
     AE_NETWORK_NAME=ae-net AE_DOCKER_NETWORK=ae-net \
     python3 -m ae.node.server --runtime-backend docker --port 9109 --advertise-endpoint http://192.168.152.11:9109 &
     ```
     (Same for worker2 with IP 192.168.152.12 and AE_NODE_ID/ADVERTISE_IP swapped.)
   - Test script (run on controller VM):
     ```
     ...
     vip=$(python3 -m ae.cli services --json | jq -r '.[0].cluster_ip')
     curl -s --max-time 5 http://$vip:8080/healthz
     # simulate node loss + reschedule
     ssh worker1 "sudo pkill -f 'ae.node.server' || true"
     python3 -m ae.cli status echo-mn --watch --timeout 120
     vip=$(python3 -m ae.cli services --json | jq -r '.[0].cluster_ip')
     curl -s --max-time 5 http://$vip:8080/healthz
