...
def _fabric_advisory_hyperon_state(
    ...
    *,
    ...
    status = "disabled"
    status_label = "none attached"
    status_message = (
        "No Hyperon/DAS evidence is attached to this advisory state; "
        "k1s advisory-only review remains available."
    )
    if enabled:
        if available:
            status = "experimental"
...
class _ApiHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_openapi(self) -> None:
        ...
        doc = {
            "openapi": "3.0.0",
            "info": {"title": "k1s Controller API", "version": "0.1.6.dev1"},
            "components": {
                "securitySchemes": {
