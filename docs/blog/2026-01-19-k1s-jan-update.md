...
AE_AGENT_API_TOKEN=changeme \
AE_AGENT_API_PORT=9110 \
python -m ae.controller --loop --specs specs/ --metrics-port 9108

# Worker agent
ae-node --runtime-backend podman --port 9109 --ensure-pod-net

# Deploy a multi-node friendly sample
python -m ae.cli apply -f specs/examples/echo-multinode.yaml
ae nodes list
ae services list
```

## 2) Service VIPs and L4 Routing

Services now receive ClusterIPs with a controller that programs backends from runtime health. For multi-replica services, k1s launches per-Service HAProxy sidecars to load balance. The Service controller deduplicates targets and skips loopback endpoints.

To enable the Service VIP dataplane, set the service proxy and provider on the controller:
...
For single-host labs, `AE_SERVICE_PROVIDER=bridge` works without WireGuard. Ingress and L4 proxies can point at the VIP, and allocations are visible via `ae services list`.

Overlay mode expects a routable overlay (WireGuard in the lab guide) so ClusterIPs are reachable across nodes, while bridge mode keeps Service VIPs host-local for single-node setups.

## 3) Kubernetes API Shim: Expanded Coverage

Shim notes:

- Pods, logs, exec, and port-forward are now wired (streaming exec with TTY resize, SPDY port-forward with websocket fallback).
- Nodes and endpoints show up, and HPA v2 is in the shim surface.
- RBAC evaluation, SubjectAccessReview, and ServiceAccount token minting are supported.
- JSONPatch and Server-Side Apply are handled, and list/watch respects resourceVersion and continue tokens.
- OpenAPI v2/v3 fixtures and drift checks are part of CI.

This does not make k1s a full Kubernetes implementation, but it removes a lot of friction for common kubectl and helm workflows.

## 4) Security and TLS: mTLS, Join Tokens, Rotation

Multi-node means trust boundaries, so we added mTLS for agent heartbeats, single-use join tokens, and cert rotation helpers. The controller keeps issued and revoked serials, and metrics expose expiry.

Quick rotation example:

```bash
ae-rotate-certs --node-id worker-1
ae certs
```

Issued and revoked cert state is persisted under `state/tls/issued.json` and `state/tls/revoked.json`.

## 5) Observability: Nodes, Placement, and Endpoint Health

Prometheus now exports node inventory, heartbeat age, and service endpoint readiness. The most useful new signals are `ae_nodes_total`, `ae_nodes_ready`, `ae_nodes_stale`, and `ae_services_total`, with per-service labels that include the ClusterIP and provider. When the API shim is enabled, `/metrics` also includes `apishim_*` watch and queue depth counters.

The dashboard picks up node health and per-node placement so you can see where replicas landed without digging through logs, and `/system` provides a combined snapshot of controller, services, and storage state. It is a quick sanity check before you open the deeper logs.

If you are running Grafana, there is a new controller-health board that focuses on node Ready/Total and service endpoint health for multi-node runs.

## 6) Tooling and Docs Updates

This cycle adds QEMU-based multi-node smoke coverage, updates lab scripts, and expands docs for the new workflows. The benchmark tooling includes memory pipeline charts and a k3s comparison harness.

Start with `docs/guides/multinode-lab.md`, then review the k8s alignment matrix in `README.md` for current shim coverage.

## What Is Next

Next up is mostly about tightening the edges and making the shim/runtime path more trustworthy for day‑to‑day use. On the API side, we want to promote the live OpenAPI gate to release‑blocking, wire the compatibility matrix + `/openapi/v3` links into docs/release notes, and expand the sample set that the live gate validates. We also want better watch propagation metrics and soak tests across shim replicas now that Postgres‑backed HA is in place.

Storage parity is the other big block of work. The plan is to add PVC/PV/StorageClass objects to the shim, build a storage controller with local‑path and NFS‑backed provisioners, and map StatefulSet `volumeClaimTemplates` into per‑ordinal runtime volumes. That is the missing piece for a lot of real Helm charts.

On the Helm/workload front, the next push is deeper fidelity for Jobs/CronJobs/StatefulSets/DaemonSets (real status fields, ordinals, per‑node scheduling) so `helm --wait` behaves closer to upstream expectations.

Finally, we want a repeatable conformance‑lite harness using Sonobuoy: establish a baseline fail list, publish artifacts, and evolve a curated profile without turning it into a hard gate too early.
