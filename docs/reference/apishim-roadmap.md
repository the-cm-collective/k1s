...
- Move shim object storage off SQLite to the primary HA authority store. Postgres remains a transitional externalized path, but the target HA end state aligns on `etcd` with the core controller; workload-core HA mutation is now on that path, and the remaining shim-native surface now narrows to the later `H4b2b` storage plus CRD/custom-resource cutover after the `H4b1`, `H4b-hpa`, and `H4b2a` slices.
