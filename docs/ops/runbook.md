...
- Fabric advisory trace review: [Fabric Advisory Trace Review](fabric-advisory-trace-review.html)
...
- OCI role images: releases now attach `k1s-core`, `k1s-core-node`, `k1s-edge-core`, and `k1s-edge-node` as linux/amd64 `*.oci.tar` archives plus `oci-image-metadata` (`SHA256SUMS` + alias manifest).
- Role alias contract: these release images are single-role runtimes, not full profile appliances.
- Podman import: `podman load -i k1s-core-<tag>-linux-amd64.oci.tar`
- Docker import: `skopeo copy oci-archive:k1s-core-<tag>-linux-amd64.oci.tar docker-daemon:k1s/k1s-core:<tag>`
