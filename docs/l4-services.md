...
Scope: How to expose non-HTTP services when running multiple replicas on one node. This complements the HTTP ingress (Caddy) guide.

Key constraints
- Caddy is an L7 HTTP/TLS proxy; it does not provide generic L4 TCP/UDP load-balancing for app ports.
- The controller assigns per-replica host ports for runtime health and connectivity; these ports are ephemeral for multi-replica apps without `spec.ingress`.
...
- External L4 proxy (recommended for multi-replica)
  - Run HAProxy or Traefik TCP on the host (or in the dev compose stack) and target the per-replica host ports that the controller publishes.
  ...
  - Example config (if editing manually):
    ...
    backend app_be
      # enumerate per-replica host ports (see `ae status` or API /status)
...
Planner hints
- When `replicas > 1` and no `spec.ingress` is present, the planner warns that per-replica host ports are ephemeral.
...
- Why a watcher? During rollouts or scale operations, per-replica host ports can change as containers are replaced. The watcher keeps HAProxy’s backend list current so your L4 entrypoint remains stable without manual edits. It validates the config (haproxy -c) inside the container before restart and reverts on failure.
