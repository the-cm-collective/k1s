...
## Step 2: Single-Host Capability Matrix

Baseline capability check across all canonical modes and workload archetypes:
...
- `core-proxy` tunnels to a fixed edge local target (`AE_EDGE_INGRESS_LOCAL_ADDR`, default `127.0.0.1:18081`).
- The matrix runner normalizes each `core-proxy` row workload service port to that fixed target so archetype checks remain mode-correct.
- Override target explicitly when needed:

```bash
scripts/dev/test_ingress_matrix_single_host.sh \
  --modes core-proxy \
  --archetypes http-static \
  --core-proxy-local-addr 127.0.0.1:18081 \
  --tier tier1
```
...
- Result JSON written under `state/test-results/ingress-matrix-<timestamp>.json`.
...
scripts/dev/test_ingress_matrix_cri.sh \
  ...
  --core-host <core-host-ip> \
  --edge-host <edge-host-ip> \
  --edge-local-listener-url https://<edge-host-ip>:11443/ \
  --modes core-proxy,core-to-edge-public,edge-local \
  --archetypes http-static,http-path-routing,http-multi-replica,http-multiport,http-redirect,http-large-payload,http2-unary \
  --tier tier1
```
...
scripts/dev/ingress_fault_injection.sh --fault gateway-restart --action cycle \
  --gateway-start-cmd "AE_SITE_ID=sea-edge-02 AE_NODE_ID=edge-1 EDGE_INGRESS_MODE=edge-local AE_EDGE_LOCAL_INGRESS_CONFIG_DIR=state/profiles/k1s-core/edge-local make k1s-edge-core"
```
