...
def test_probe_history_roundtrip(tmp_path: Path) -> None:
    ...
    with store._connect() as conn:  # type: ignore[attr-defined]
        conn.execute(
            """
            INSERT INTO probe_history(app_name, pod_name, check_time, ready, live, readiness_message, liveness_message)
            VALUES(?,?,?,?,?,?,?)
            """,
            (
                "echo",
    ...
    assert out and out[0].pod_name == "echo-0" and out[0].ready and out[0].live
