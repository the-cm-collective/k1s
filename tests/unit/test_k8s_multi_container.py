...
def test_export_multi_container_and_initcontainers() -> None:
    ...
    c2 = _AppSpec.ContainerSpec(
        ...
        lifecycle=LifecycleSpec(
            postStart=LifecycleHandler(exec={"command": ["/bin/sh", "-c", "echo sidecar"]})
        ),
    )
    init = {"name": "init", "image": man.spec.image, "command": ["/bin/sh", "-c", "echo init"]}
    man = man.model_copy(
        update={
            "spec": man.spec.model_copy(update={"containers": [c1, c2], "init_containers": [init]})
        }
    )
...
def test_projection_mounts_per_init_container() -> None:
    ...
    assert any(
        m.get("name") == vname and m.get("subPath") == "secret/token" and m.get("readOnly")
        for m in mounts
    )
