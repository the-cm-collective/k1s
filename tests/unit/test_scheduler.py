...
def test_scheduler_storage_pins_single_node(tmp_path):
    ...
    store.upsert_node("n2", name="n2", labels={}, taints=[], backend="podman", endpoint="http://n2:9109")
    store.record_heartbeat("n1", "Ready")
    store.record_heartbeat("n2", "Ready")
    man = _manifest(replicas=2)
    man = man.model_copy(update={"spec": man.spec.model_copy(update={"storage": [{"name": "data", "mountPath": "/data"}]})})
    sched = Scheduler(store)
...
def test_scheduler_filters_cordoned_and_stale(tmp_path, monkeypatch):
    ...
    store.record_heartbeat("n2", "Ready")
    # stale node n3
    store.upsert_node("n3", name="n3", labels={}, taints=[], backend="podman", endpoint="http://n3:9109")
...
def test_scheduler_spreads_by_topology_key(tmp_path):
    ...
    man = man.model_copy(update={"spec": man.spec.model_copy(update={"topology_spread_constraints": constraints})})
    sched = Scheduler(store)
    placements, warnings = sched.plan(man, revision=1)
    assert not warnings
