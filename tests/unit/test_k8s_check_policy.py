...
def test_policy_escalation_to_strict() -> None:
    ...
    man = man.model_copy(update={"spec": man.spec.model_copy(update={"replicas": 2, "resources": None})})
...
def test_canary_single_replica_warns() -> None:
    ...
    man = man.model_copy(update={"spec": man.spec.model_copy(update={"replicas": 1, "rollout": {"strategy": "canary"}})})
    issues = k8s_portability_issues(man)
...
spec:
  ...
  replicas: 2
    ...
    exit_code = main(["k8s-check", "-f", str(man_path), "--assume-hpa", "cpu-util", "--assume-hpa", "mem-value=200Mi", "--policy", "strict"])
    assert exit_code != 0
...
def test_hpa_mem_value_invalid(tmp_path) -> None:
    ...
    exit_code = _main(["k8s-check", "-f", str(man_path), "--assume-hpa", "mem-value=20XYZ", "--policy", "strict"])
    assert exit_code != 0
