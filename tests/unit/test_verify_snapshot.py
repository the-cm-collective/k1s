...
def test_verify_snapshot_summarize_matches_summary(tmp_path: Path) -> None:
    ...
    (snap / "meta.json").write_text(json.dumps({"mode": "k1s", "label": "pods-1", "timestamp": "t"}))
    ...
    (raw / "podman_inspect.json").write_text(json.dumps([
        {"Id": "abc123def4560000", "Name": "ae-echo", "Config": {"Labels": {"ae.app": "echo"}}},
        {"Id": "fff1112223339999", "Name": "caddy", "Config": {"Labels": {}}},
    ]))

    # synthetic summary.json to compare with
    (snap / "summary.json").write_text(json.dumps({
        "containers": {
