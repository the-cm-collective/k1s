...
def _make_handler(tmp_path):
    ...
    obj.send_header = lambda k, v: _hdr(k, v)  # type: ignore[attr-defined]
    ...
    # minimal helpers
    obj._json_ok = lambda payload: _send(200)  # type: ignore[attr-defined]
    obj._json_error = lambda code, msg: _send(code)  # type: ignore[attr-defined]
    return obj
...
def test_k8s_preview_dev_gate_and_success(monkeypatch, tmp_path: Path):
    ...
    handler.rfile.read = lambda n: b"{}"  # type: ignore[attr-defined]
    ...
    handler.rfile.read = lambda n: body  # type: ignore[attr-defined]
