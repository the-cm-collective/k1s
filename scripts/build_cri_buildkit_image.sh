...
usage() {
  cat <<'USAGE'
Usage: scripts/build_cri_buildkit_image.sh [options]

Options:
  --registry <host:port>  Registry host (defaults to AE_REGISTRY_HOST)
  --tag <name:tag>        Image name/tag (default: k1s-buildkit:latest)
...
$engine build -f ops/images/cri-buildkit.Dockerfile -t "$image" "${build_args[@]}" .
