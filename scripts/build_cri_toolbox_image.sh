...
usage() {
  cat <<'USAGE'
Usage: scripts/build_cri_toolbox_image.sh [options]

Options:
  --registry <host:port>  Registry host (defaults to AE_REGISTRY_HOST)
  --tag <name:tag>        Image name/tag (default: k1s-cri-toolbox:latest)
  --push                  Push after build
  --engine <docker|podman> Build engine (defaults: podman then docker)
  -h, --help              Show this help

Environment (optional):
  CRICTL_VERSION, NERDCTL_VERSION, BUILDKIT_VERSION
...
if [[ -n "${NERDCTL_VERSION:-}" ]]; then
  build_args+=(--build-arg "NERDCTL_VERSION=${NERDCTL_VERSION}")
fi
if [[ -n "${BUILDKIT_VERSION:-}" ]]; then
  build_args+=(--build-arg "BUILDKIT_VERSION=${BUILDKIT_VERSION}")
fi

$engine build -f ops/images/cri-toolbox.Dockerfile -t "$image" "${build_args[@]}" .
