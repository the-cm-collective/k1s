...
usage() {
  cat <<'USAGE'
Usage: scripts/build_cri_crictl_image.sh [options]

Options:
  --registry <host:port>  Registry host (defaults to AE_REGISTRY_HOST)
  --tag <name:tag>        Image name/tag (default: k1s-crictl:latest)
...
$engine build -f ops/images/cri-crictl.Dockerfile -t "$image" "${build_args[@]}" .
