...
for row in "${rows[@]}"; do
  ...
  if ! wait_for_ssh "$ip" 40; then
    rec="$(jq -n --arg name "$name" --arg ip "$ip" '{name:$name,ip:$ip,ssh:false}')"
    results="$(jq --argjson r "$rec" '. + [$r]' <<<"$results")"
    continue
  fi

  cloud_init="$(run_remote "$ip" "cloud-init status --wait >/dev/null 2>&1; echo ok" || true)"
  ...
  rec="$(jq -n \
    ...
    --arg gpu "$gpu_status" \
    --arg cri "$cri_status" \
    '{name:$name,ip:$ip,ssh:true,cloud_init:$cloud_init,gpu_check:$gpu,cri_preflight:$cri}')"
  results="$(jq --argjson r "$rec" '. + [$r]' <<<"$results")"
done
